...
% warp_im = imtransform( im, tform, 'bilinear', 'FillValues', fill_value*ones(size(im,3),1));

% warp_im = imtransform( im, tform, 'bilinear', 'Size', out_size(1:2), 'FillValues', fill_value*ones(size(im,3),1));

% warp_im = imtransform( im, tform, 'bilinear', 'XData', [1 out_size(2)], ...
%     'YData', [-119 695], 'FillValues', fill_value*ones(size(im,3),1));

% warp_im = imtransform( im, tform, 'bilinear', 'XData', [1 out_size(2)*2], ...
%     'YData', [1 out_size(1)], 'FillValues', fill_value*ones(size(im,3),1));
